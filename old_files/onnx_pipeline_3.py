...
class OnnxFraudService:
    ...
    def _run_inference(self, features: Dict, return_id: int, order_id: int, client_id: int) -> Dict:
        ...
        try:
            ...
            raise RuntimeError(f"Ошибка One-Hot Encoding: {e}")
        ...
        # CatBoost predict_proba (реальные вероятности)
        proba = self.pattern_model.predict_proba(X_df)[0]
        prob_fraud = float(proba[1])  # P(fraud)
        ...
        combined_score = 0.3 * prob_fraud + 0.7 * rule_score + 1 * anomaly_risk
        ...
        return {
            "return_id": int(return_id), "client_id": int(client_id), "order_id": int(order_id),
            "probability_fraud": round(prob_fraud, 4), "anomaly_score": round(anomaly_score, 4),
            "is_anomaly": bool(is_anomaly), "combined_score": round(combined_score, 4),
